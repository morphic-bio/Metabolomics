...
${SEQUENCE_DIR} - Directory containing the sequence CSV files to be combined.
${OUTPUT_DIR} - Directory where the combined CSV file will be saved.
(Use `export` (e.g.: `export SEQUENCE_DIR=/path/to/your/sequences`) to set variables in your shell)

Run via `python3.11 step1_combine_sequences.py`
"""
...
def combine_sequences(directory):
   ...
   print(f"Combining sequence files from directory: {directory}")
   for file in os.listdir(directory):
       if file.endswith(".csv"):
           if "combined_sequences.csv" in file or "combined_sequences_modified.csv" in file or "pcpfm_sequence.csv" in file:
               continue
   ...
   output_file = os.getenv('OUTPUT_DIR') + "/combined_sequences.csv"
...
def main():
   ...
   directory = os.getenv('SEQUENCE_DIR') # path to directory containing CSV sequence files
